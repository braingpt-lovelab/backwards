...
def first_map_then_remove_last(config_fpath):
    ...
    # If `args.cache_dir_train` exist,
    # do nothing.
    if os.path.exists(args.cache_dir_train):
        print(f"\n\nCache already exists at {args.cache_dir_train}.")
        print("Exiting...\n\n")
        return None, None
...
if __name__ == "__main__":
    ...
    for i in range(tokenized_dataset["train"].num_rows):
        if len(tokenized_dataset["train"][i]["input_ids"]) != 1024:
            print(i, len(tokenized_dataset["train"][i]["input_ids"]))
        if i % 100 == 0:
            print(
                i, tokenizer.convert_ids_to_tokens(tokenized_dataset["train"][i]["input_ids"])[:5]
            )