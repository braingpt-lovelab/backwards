...
def tokenize(element, tokenizer, args):
    ...
    # Reverse input_ids (abstract runs backwards using implicitly reversed vocab).
    if "fwdModel_bwdText" in results_dir:
        outputs["input_ids"] = \
            [torch.flip(torch.tensor(x), dims=[0]) for x in outputs["input_ids"]]
...
if __name__ == "__main__":
    ...
    for llm in llms:
        ...
        results_dir = \
            f"model_results/{llm.replace('/', '--')}_fwdModel_bwdText/{type_of_abstract}"
