...
def ppl_distributional_diff():
    ...
    for model_pair_idx, model_pair in enumerate(model_pairs):
        ...
        # T-test
        # (paired t only valid for bayes trained models in that the test items match exactly,
        # for original backwards trained models, due to retrained tokenizer, and padding, the total
        # number of validation samples are different)
        if "bayes" in forwards_model_fullname:
            t_stat, p_val = stats.ttest_rel(forwards_val_ppl, backwards_val_ppl)
            print(f"Validation PPL: {forwards_model_print_name} vs {backwards_model_print_name}")
    ...
    axes[-1].legend() 
    plt.tight_layout()
    plt.savefig(f"{figs_dir}/ppl_distributional_diff_val_and_test_with_bayes.pdf")
