...
def compute_attention_weights_by_distance(attention_weights):
    """
    Compute attention weights by distance and mean attention weights by column.
    ...
    Returns:
        ...
        mean_weights (torch.Tensor): Mean attention weights for each unique distance.
        col_mean_weights (torch.Tensor): Mean attention weights for each column, shape (seq_len,).
        head_col_weights (torch.Tensor): Attention weights for each head and column, shape (n_heads, seq_len).
    """
    ...
    # Compute mean attention weights for each unique distance
    unique_distances = torch.unique(distances)
    mean_weights = torch.zeros(len(unique_distances), device=device, dtype=dtype)
    for dist in unique_distances:
        mask = distances == dist
        mean_weights[dist] = weights[:, :, mask].mean()

    # Compute mean attention weights for each column (averaged over heads)
    col_mean_weights = torch.zeros(seq_len, device=device, dtype=dtype)
    for col_idx in range(seq_len):
        mask = j == col_idx
        col_mean_weights[col_idx] = weights[:, :, mask].mean()

    # Compute attention weights for each head and column (averaged over batches)
    head_col_weights = torch.zeros(n_heads, seq_len, device=device, dtype=dtype)
    for col_idx in range(seq_len):
        mask = j == col_idx
        head_col_weights[:, col_idx] = weights[:, :, mask].mean(dim=(0,2)).squeeze()

    return unique_distances, mean_weights, col_mean_weights, head_col_weights
...
def get_attention_weights_n_entropy_per_batch_mean_head(
    ...
    for layer_index in range(n_layers):
        ...
        # Compute per head weights, col weights, and individual head col weights
        unique_distances, mean_weights, col_weights, head_col_weights = compute_attention_weights_by_distance(attention_weights1)

        # Store in preallocated tensors
        attn_weights_x_batches["fwd"][layer_index]["unique_distances"] = unique_distances
        attn_weights_x_batches["fwd"][layer_index]["mean_weights"] = mean_weights
        attn_weights_x_batches["fwd"][layer_index]["mean_head_weights_col_weights"][batch_index] = col_weights
        attn_weights_x_batches["fwd"][layer_index]["head_weights_col_weights"][batch_index] = head_col_weights

    return attn_weights_x_batches


def visualize_attention_weights_col_weights(attn_weights_x_batches):
    n_layers = len(attn_weights_x_batches["fwd"])
    n_cols = 6
    ...
    for layer_index in range(n_layers):
        ...
        ax.plot(
            ...
            attn_weights_x_batches["fwd"][layer_index]["mean_weights_col_weights"],
    ...
    plt.legend()
    plt.tight_layout()
    fig_fpath = f'figs/attn_weights_col_weights_by_distance_{model_size}_seed{model_seed}_seed{random_seed}.png'
    if "neuroscience" not in dataset:
        fig_fpath = f'figs/attn_weights_col_weights_by_distance_{model_size}_seed{model_seed}_seed{random_seed}_{dataset}.png'
    plt.savefig(fig_fpath)
    print(f"Saved attention weights col weights by distance plot to disk: {fig_fpath}")


def visualize_individual_head_col_weights(attn_weights_x_batches):
    n_layers = len(attn_weights_x_batches["fwd"])
    n_heads = attn_weights_x_batches["fwd"][0]["head_weights_col_weights"].shape[0]
    ...
    for layer_index in range(n_layers):
        for head_idx in range(n_heads):
            ...
            ax.plot(
                range(attn_weights_x_batches["fwd"][layer_index]["head_weights_col_weights"].shape[-1]),
                attn_weights_x_batches["fwd"][layer_index]["head_weights_col_weights"][head_idx, :],
    ...
    plt.tight_layout()
    fig_fpath = f'figs/attn_weights_head_col_weights_{model_size}_seed{model_seed}_seed{random_seed}.png'
    if "neuroscience" not in dataset:
        fig_fpath = f'figs/attn_weights_head_col_weights_{model_size}_seed{model_seed}_seed{random_seed}_{dataset}.png'
    plt.savefig(fig_fpath)
    plt.close(fig)
    print(f"Saved individual head col weights plot to disk: {fig_fpath}")
...
def main():
    ...
    if not os.path.exists(result_fpath):
        ...
        for model_key in attn_weights_x_batches:
            for layer_index in attn_weights_x_batches[model_key]:
                ...
                # Average over batches for mean head col weights
                attn_weights_x_batches[model_key][layer_index]["mean_weights_col_weights"] = torch.mean(
                    attn_weights_x_batches[model_key][layer_index]["mean_head_weights_col_weights"],
                    dim=0,
                )
                # Average over batches for individual head col weights
                attn_weights_x_batches[model_key][layer_index]["head_weights_col_weights"] = torch.mean(
                    attn_weights_x_batches[model_key][layer_index]["head_weights_col_weights"],
                    dim=0,
                )
    ...
    visualize_attention_weights_col_weights(attn_weights_x_batches)
    visualize_individual_head_col_weights(attn_weights_x_batches)
