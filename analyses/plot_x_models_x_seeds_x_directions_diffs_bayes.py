...
def compute_stats(model1_ppls, model2_ppls):
    ...
    pearson_r, pearson_r_p_val = stats.pearsonr(model1_ppls, model2_ppls)
    cohen_d = (np.mean(model1_ppls) - np.mean(model2_ppls)) / np.sqrt(
        np.var(model1_ppls - model2_ppls, ddof=1)
    )
    return t_stat, p_val, pearson_r, pearson_r_p_val, cohen_d
        ...
        for direction, model_names in per_direction_models.items():
            ...
            for i in range(len(model_names)):
                for j in range(i + 1, len(model_names)):
                    ...
                    # Compute stats
                    t_stat, p_val, pearson_r, pearson_r_p_val, cohen_d = compute_stats(model1_ppls, model2_ppls)
                    ...
                    per_direction_model_pairs_stats["pearson_r_p_val"].append(pearson_r_p_val)
            ...
            avg_pearson_r_p_val = np.mean(per_direction_model_pairs_stats["pearson_r_p_val"])
            ...
            std_pearson_r_p_val = np.std(per_direction_model_pairs_stats["pearson_r_p_val"])
            ...
            print(f"      pearson_r: {avg_pearson_r:.3f} ± {std_pearson_r:.3f}")
            print(f"      pearson_r_p_val: {avg_pearson_r_p_val:.3f} ± {std_pearson_r_p_val:.3f}")
            print(f"      t_stat: {avg_t_stat:.3f} ± {std_t_stat:.3f}")
            print(f"      p_val: {avg_p_val:.3f} ± {std_p_val:.3f}")
            print(f"      cohen_d: {abs(avg_cohen_d):.3f} ± {std_cohen_d:.3f}")
    ...
    for model_family, per_direction_models in comparison.items():
        ...
        for direction_pair, model_pairs in per_direction_models.items():
            ...
            for model_pair in model_pairs:
                ...
                per_direction_pair_model_pairs_stats["pearson_r_p_val"].append(pearson_r_p_val)
            ...
            avg_pearson_r_p_val = np.mean(per_direction_pair_model_pairs_stats["pearson_r_p_val"])
            ...
            std_pearson_r_p_val = np.std(per_direction_pair_model_pairs_stats["pearson_r_p_val"])
