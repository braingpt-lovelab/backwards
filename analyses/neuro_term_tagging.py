...
async def load_existing_results(fname):
    ...
    try:
        ...
        async with aiofiles.open(f"{results_dir}/{fname}__neuro_term_tagging_results.txt", "w") as f:
            pass
    return results
...
async def main(fname):
    ...
    if "backwards" in fname:
        forwards_results = await load_existing_results(fname="forwards_filtered")
    total_cost = 0
    ...
    async with aiofiles.open(f"{data_dir}/{fname}.txt", "r") as f:
        ...
        for line in tqdm.tqdm(lines):
            ...
            if "backwards" in fname and word in forwards_results:
                print(f"{word} same as forwards, copy over to backwards")
                async with aiofiles.open(f"{results_dir}/{fname}__neuro_term_tagging_results.txt", "a") as f:
                    await f.write(f"{word}, {forwards_results[word]}\n")
                continue
